...
function DocumentConversion(options, comments) {
  ...
  function findStyle(element) {
    if (options.dynamicStyleMap) {
      var dynamic = options.dynamicStyleMap(element);
      if (dynamic) {
        var converted =
          element.type[0] + "." + element.styleId + " => " + dynamic;

        var rule = StyleReader.readStyle(converted);
        if (rule.value) {
          return rule.value;
        }
      }
  ...
  var elementConverters = {
    ...
    horizontalRule: function (element, messages, options) {
      return [Html.freshElement("hr", {}, [])];
    },
